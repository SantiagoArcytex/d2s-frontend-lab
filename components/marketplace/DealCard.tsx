...
export function DealCard({ deal }: DealCardProps) {
  ...
  return (
    <Link href={`/marketplace/${deal.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
      <Card
        variant="elevated"
      >
        <div 
          ...
          onMouseEnter={(e) => {
            const card = e.currentTarget.closest('[class*="MuiCard"]') as HTMLElement;
            if (card) {
        ...
        >
          ...
          <Text
            variant="body"
            style={{
              color: deal.description ? 'var(--text-secondary)' : 'var(--text-muted)',
              fontSize: '0.875rem',
              lineHeight: 1.5,
